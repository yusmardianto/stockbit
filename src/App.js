...
const ColCardBox = ({ Title, imdbID, Poster, Type, ShowDetail, DetailRequest, ActivateModal }) => {
  ...
  const clickHandler = () => {
    ...
    axios
      .get(
        `http://www.omdbapi.com/?i=${imdbID}&apikey=${API_KEY}&_limit=5`
      )
      .then(resp => resp.data)
      .then(response => {
        DetailRequest(false);
...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://www.omdbapi.com/?s=${q}&apikey=${API_KEY}&_limit=5`
      )
      .then(resp => resp.data)
      .then(response => {
      ...
      <Layout className="layout">
        <Header>
          <div style={{ textAlign: 'center' }}>
            <TextTitle style={{ color: '#ffffff', marginTop: '14px' }} level={3}>Stockbit</TextTitle>
          </div>
        </Header>
        ...
        <Footer style={{ textAlign: 'center' }}>Stockbit</Footer>
      </Layout>
    </div>
  );
}
